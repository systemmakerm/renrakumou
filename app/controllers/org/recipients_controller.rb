class Org::RecipientsController < ApplicationController

  layout :select_rec_layout

  NORMALACTIONS = ['detail','dissolve','decide_dissolve']

  SIMPLEACTIONS = ['number_check','number_confirm','dissolve_complete',
                   'show', 'edit','update_before_confirm','update',
                   'recipient_complete','new', 'create_before_confirm',
                   'create', 'editor_user','user_up_before_confirm','user_up',
                   'editor']

  def select_rec_layout
    if SIMPLEACTIONS.include?(params[:action])
      return 'simple'
  ...
  def editor
    @account = current_account
    @user = @account.recipient.user
    @group_ids = current_account.recipient.recipient_group_check
    render 'recipient_form'
  end
...
#      return redirect_to recipient_complete_org_recipients_path(:id => @account.id)
      return redirect_to recipient_complete_org_recipients_path
    else
      @account.errors.full_messages
      render 'recipient_form'
    end
